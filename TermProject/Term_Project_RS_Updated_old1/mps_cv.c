...
void enqueue(MutexQueue *mutex_queue, Burst *burst);
Burst *dequeue(MutexQueue *mutex_queue);
...
// New enqueue function for Queue type
void enqueue_queue(Queue *queue, Burst *burst) {
    if (queue->tail == NULL) {
        queue->head = burst;
        queue->tail = burst;
    } else {
...
void read_input_file() {
    ...
    while (fscanf(file, "%d %d %d", &pid, &burst_length, &arrival_time) == 3) {
        ...
        if (sap == 'S') {
            ...
            enqueue(&mutex_queues[target_queue], burst);
        }
    }
...
void enqueue(MutexQueue *mutex_queue, Burst *burst) {
    ...
    if (mutex_queue->queue.tail == NULL) {
        mutex_queue->queue.head = burst;
        mutex_queue->queue.tail = burst;
    } else {
        mutex_queue->queue.tail->next = burst;
        mutex_queue->queue.tail = burst;
    }
    burst->next = NULL;
    pthread_cond_signal(&mutex_queue->cond);
    pthread_mutex_unlock(&mutex_queue->lock);
}

Burst *dequeue(MutexQueue *mutex_queue) {
    pthread_mutex_lock(&mutex_queue->lock);
    while (is_queue_empty(&mutex_queue->queue)) {
        pthread_cond_wait(&mutex_queue->cond, &mutex_queue->lock);
    }
    Burst *burst = mutex_queue->queue.head;
    if (burst != NULL) {
        mutex_queue->queue.head = burst->next;
        if (mutex_queue->queue.head == NULL) {
            mutex_queue->queue.tail = NULL;
        }
    }
    pthread_mutex_unlock(&mutex_queue->lock);
    return burst;
...
void *processor_thread(void *arg) {
    ...
    while (true) {
        Burst *burst = dequeue(mutex_queue);
        ...
        if (burst != NULL) {
            ...
            // Use the new function for finished_bursts
            enqueue_queue(&finished_bursts, burst);
            // enqueue(&finished_bursts, burst);
            pthread_mutex_unlock(&finished_bursts_lock);
            pthread_mutex_lock(&completed_processes_lock);
            completed_processes++;
            pthread_mutex_unlock(&completed_processes_lock);
        }
