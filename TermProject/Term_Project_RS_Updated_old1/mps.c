...
typedef struct burst {
    ...
    int finish_time;
    int turnaround_time;
    int waiting_time; // Add
    int response_time; // Add
    int start_time; // Add
    int cpu_id;
    struct burst *next;
} Burst;
...
void *processor_thread(void *arg) {
    ...
    while (true) {
        ...
        if (burst != NULL) {
            ...
            burst->waiting_time = burst->finish_time - (burst->arrival_time + burst->burst_length); // Add
	    burst->response_time = burst->finish_time - burst->arrival_time; // Add
...
void output_simulation_results() {
    ...
    if (outmode == 1 || outmode == 3) {
        ...
        printf("BurstID\tCPU_ID\tArrival_Time\tFinish_Time\tTurnaround_Time\tWaiting_Time\tResponse_Time\n");
    }
    if (outmode == 2 || outmode == 3) {
        ...
        fprintf(output_file,"BurstID\tCPU_ID\tArrival_Time\tFinish_Time\tTurnaround_Time\tWaiting_Time\tResponse_Time\n");
    }

    for (Burst *burst = finished_bursts.head; burst != NULL; burst = burst->next) {
        if (outmode == 1 || outmode == 3) {
            printf("%d\t%d\t%.2f\t\t%.3f\t\t%.2f\t\t%.2f\t\t%.2f\n", burst->pid, burst->cpu_id,
                (double)burst->arrival_time, (double)burst->finish_time, (double)burst->turnaround_time, (double)burst->waiting_time, (double)burst->response_time);
        }
        if (outmode == 2 || outmode == 3) {
            fprintf(output_file, "%d\t%d\t%.2f\t\t%.3f\t\t%.2f\t\t%.2f\t\t%.2f\n", burst->pid, burst->cpu_id,
                (double)burst->arrival_time, (double)burst->finish_time, (double)burst->turnaround_time, (double)burst->waiting_time, (double)burst->response_time);
        }
    }
