...
/*
	These task parametrs are given by the user
	...
	T[4] == T[abs_arrival] 		(Absulute Arrival time)
	T[5] == T[execution_copy] 	(Execution time copy)
	T[6] == T[abs_deadline]		(Absuluta Ariival time)
	
	instance (Number of times the tasks had arrived since time = 0)
	alive (Whether the task is ready? - for NOT READY and 1 for READY)
*/
} task;
...
void update_abs_arrival(task *t1, int n, int k, int all);   // Update absolute arrival time (absolute arrival time = arrivaltie + instance*period)
